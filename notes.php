<?php
// Uses existing $conn, $helper, $security from index.php
$roleId = $_SESSION['LOGIN_ROLE_ID'] ?? 0;
...
if (isset($_POST['add_note'])) {
    ...
    if ($caseId && $title && $details) {
        $noteId = $helper->generateNextNotesID(); // Use helper function
        $stmt = $conn->prepare("INSERT INTO notes (note_id, case_id, title, details, created_date, updated_date, is_deleted, role_id) VALUES (?, ?, ?, ?, NOW(), NOW(), 0, ?)");
        $stmt->bind_param("ssssi", $noteId, $caseId, $title, $details, $roleId);
        $stmt->execute();
        $stmt->close();
