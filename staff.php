<?php
	...
	if(isset($_POST["btn_edit"])){
		$txt_staff_id = mysqli_real_escape_string($conn, $_GET["staff_id"]);
		$txt_first_name = mysqli_real_escape_string($conn, $_POST["txt_first_name"]);
		$txt_last_name= mysqli_real_escape_string($conn, $_POST["txt_last_name"]);
		$int_mobile_number= mysqli_real_escape_string($conn, $_POST["int_mobile_number"]);
		$txt_nic_number= mysqli_real_escape_string($conn, $_POST["txt_nic_number"]);
...
<html lang="en">
	...
	<body>
							...
							<table class="table table-responsive table-striped attractive-table">
								...
								<tbody>
									...
									<tr>
										...
										<a href="http://localhost/ucms/index.php?pg=staff.php&option=delete&staff_id=<?php echo htmlspecialchars($row['staff_id']); ?>" class="btn btn-danger btn-sm">
		...
		<div class="container mt-4">
			...
			<?php
			// <!-- EDIT SECTION -->
			}elseif(isset($_GET['option']) && $_GET['option'] == "edit" && $_GET['staff_id']) {
				$data = getStaffDataFromDatabase(htmlspecialchars($_GET['staff_id']), $conn);
				// print_r($data);
			...
			<div class="row justify-content-center">
				<div class="col-md-8 col-lg-6">
					<form action="#" method="POST" id="staffForm">
						<div class="row mb-3">
							<label for="staff_id" class="form-label">Staff ID</label>
							<input type="text" class="form-control" id="txt_staff_id" name="txt_staff_id" value="<?php echo htmlspecialchars($_GET['staff_id']); ?>" readonly>
						</div>
						<div class="row mb-3">
							<div class="col-md-6">
								<label for="first_name" class="form-label">First Name</label>
						...
						<button type="submit" class="btn btn-primary" id="btn_edit" name="btn_edit">Submit</button>
						<button type="button" class="btn btn-secondary" id="btn_clear" name="btn_clear">Clear Inputs</button>
					</form>
				</div>
			</div>
