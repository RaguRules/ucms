...
require_once ('lib/security.php');
	...
	if (isset($_POST["btn_add"])) {
	    ...
	    if ($resultInsert) {
			...
			$sqlInsert = "INSERT INTO `courtsmanagement`.`login` (`username`, `password`, `otp`, `status`, `role_id`) VALUES ('$txt_email', '$hashedPassword', '666666', '$status', '$select_role_name');";
			$resultInsert = mysqli_query($conn, $sqlInsert) or die("Error in sqlInsert: " . mysqli_error($conn));
			if ($resultInsert) {
				...
				//need to remove from staff table	
				$sqlDelete = "DELETE FROM staff WHERE email = '$txt_email'";
...
<div class="card-footer bg-light rounded-bottom-4">
  <div class="d-flex justify-content-center gap-2">
	...
	<button type="button" class="btn btn-danger" onclick="deleteConfirmModal(() => this.closest('form').submit())">
		<i class="fas fa-trash-alt"></i> Delete
	</button>
</form>
		...
		<?php
			// <!-- DELETE SECTION -->
					...
					if ($resultDelete) {
						...
						echo '<script>alert("Staff member deleted successfully.."); window.location.href="index.php?pg=staff.php&option=view";</script>';
					} else {
						echo '<script>alert("Error deleting staff member: ' . mysqli_error($conn) . '");</script>';
					}
